...
| 渲染器能力 | `src/canvas/renderers/registry.ts` | editable / saveStrategy / defaultExt（纯函数） |
| 渲染器 UI | `src/components/Canvas/renderer-ui-registry.ts` | Vue 组件 + 图标映射 |
| 领域逻辑 | `src/canvas/artifact-registry.ts` | 纯函数 registry |
| UI 适配 | `src/stores/canvas.ts` | Pinia tab 容器 + 布局比例 + 溯源跳转 |
| 保存逻辑 | `src/canvas/artifact-actions.ts` | Save / Save As / Save All（查注册表） |
...
| 右键菜单 | `artifact-context-menu.ts` | 菜单项可见性（查 editable） |
...
1. `src/canvas/renderers/registry.ts` — 能力（editable / saveStrategy / defaultExt）
2. `src/components/Canvas/renderer-ui-registry.ts` — 组件 + 图标

`CanvasPanel` 通过 `<component :is="getRendererComponent(type)">` 动态渲染。
...
- `src/canvas/__tests__/renderer-registry.test.ts`
- `src/canvas/__tests__/artifact-source.test.ts`
