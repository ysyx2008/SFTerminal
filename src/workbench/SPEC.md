...
- **助手换台（2026-08-18 确认）**：独立助手默认是对话台（对话在主位，打开的文件在右边）。助手开了真终端后，**同一场对话换到终端台**——左边是透明终端，右边是原来这场对话。关掉最后一扇终端则滑回对话台，人不散场。这不是把终端当作又一份往右开的产出物，也不把这场对话改成终端页。终端页（本机/远程）自己的关页即散场，不受影响。
- **换台后每栏都有顶栏（2026-08-18 确认）**：左边终端栏也要有自己的顶栏，和右边对话栏齐平。顶栏可以拖动窗口。关掉终端（标题栏关掉，或关掉最后一扇）就滑回对话台，人不走。侧栏收起时顶栏必须给窗口按钮让位，标题不能被挡住。
- **侧栏开合只让最左那一栏给窗口按钮让位（2026-08-19 确认）**：换台后左边是终端、右边是对话。折叠历史对话侧栏时，只有贴着窗口左沿的那一栏（终端标题栏）给红绿灯 / 侧栏开关让位；右边对话栏不贴窗口左沿，顶栏上的「严格/宽松/自由」和主机信息必须留在对话栏自己的顶栏里，不要跟着侧栏跑。没开终端、对话独占主区时，对话顶栏才吃左侧让位。
- **独立助手的终端只要标题、不要标签条（2026-08-19 确认）**：一个对话一个 Agent，多开标签没有意义，连标签条都不要。换到终端台后，左边顶栏只要一个标题（如「本地终端」或 SSH 名），不要终端页那种标签条、不要加号、不要左右滚动。标题栏要能关，或留着明显的关闭；关了仍滑回对话台。不要把这场对话改成终端页。终端页（本机 / 远程）仍然可以多开标签，每个标签一个 Agent，那边不要动。同一场对话里把窗口分屏可以留（还是这一个 Agent 的多扇窗）。
- **换台分隔线与主机信息（2026-08-19 确认）**：换到终端台后，左边终端和右边对话之间要有和终端页一样的竖向分隔线。关掉终端滑回对话台后，这台终端的主机信息不要留着。
- **终端页也是三栏（2026-08-19 确认）**：侧栏、终端、对话各占一栏，各栏自己的顶栏贴到窗口上沿。不要再在两栏上头压一条通栏菜单。独立助手开了真终端之后，看上去就应该和终端页一样。对话栏的折叠按钮钉在窗口上，不跟着栏走，和历史侧栏那个开关一样。钉住之后还要能点；位置和第一排对齐，并让开窗口控件。
