...
> Last verified: 2026-08-28（大文件解析进度不要撑高输入框）  
...
> - **问题**：技能装没装上，输入的人看不见。
> - **成功标准**：
>   1. @ 里能选已启用的技能（内置的和自己写的）；选中后立刻出现胶囊，不进输入文字。
>   2. 胶囊单独一排，不要标题。不和这一条要发出去的文件、摘录混在一起。多了就在这一排里左右划、按住拖；两边淡出，让人知道后面还有。刚加上的划进眼前。
>   3. 整场对话一直挂着；秘书自己装上的也在这排。点掉就没了。悬停一会儿才出简介；变灰的说明也一样，不要一碰上就跳出来。
>   4. 新开上的那一下可以轻轻亮一下，已经开着的不要每条消息都闪。
>   5. 这场里用过、现在已经关掉或没有了的，仍留在这一排，但变灰；悬停说明这是以前用过、现在没有了。点掉就没了。
> - **关键取舍**：这一排表示「这场对话现在开着」，不是「这一条要发出去的附件」。多了也不往下长、不收进「还有几个」。装不上的不要装成还开着，也不要悄悄拿走让人以为从没开过。
