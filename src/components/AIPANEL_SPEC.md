...
> Last verified: 2026-08-26（推荐只标注、倒计时跟问题同一行）  
...
>   2. 等你回答时，倒计时写在问题同一行右侧，不另占一行、也不画在选项上。超时或取消时才在底下说出原因。
> - **关键取舍**：稳比多一行提示重要。
> - **明确不做**：不因此改超时本身，也不靠回车代选。
>
> **提问框标出最推荐的选项（设计目标，2026-08-26）**
>
> - **问题**：选项都长一个样，用户还得自己比较哪个更合适。整行高亮会和悬停抢颜色，看起来像已经选了。
> - **成功标准**：
>   1. 最推荐的那一个排在最前，并标出「推荐」。
>   2. 只标注，不把整行涂亮。
> - **关键取舍**：推荐写在选项上，不另起一行重复那句话。
> - **明确不做**：不替 Agent 猜哪个该推荐；不靠回车代选。
>
