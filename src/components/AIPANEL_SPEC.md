...
> - **成功标准**：
>   1. 一个任务在界面上只有两种东西：**它说给你听的话永远在外面**（对话主体），**它做事的过程永远收着一行**（附属）。
>   2. 那一行从任务开始到结束**位置不动、行数不变，只换内容**——跑着的时候说它在忙什么并走秒，做完说这段做了什么、花了多久。不是"做完才收"，而是压根没展开过，所以没有任何东西冒出来又消失。
>   3. 跑着时那句"在忙什么"用**它自己刚写下的思考**，没有才退回动作分类。像隔着门喊一句"我在查上个月的账"，而不是报工具名。
...
>   7. 点开任一行，原来的底稿都还在，顺序与它当时干活的顺序一致；想一直看全过程的人，可以在设置里把自动收起关掉。
