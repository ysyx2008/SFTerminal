...
4. **遵循原则**：
   ...
   - **禁止代码实现内容**：类名、函数名、变量名、文件路径、文件清单、API 表、执行流程伪代码一律不进 SPEC——这些是 AI 的实现自由裁量空间，真相源是代码和 CodeGraph
...
2. **逐个检查**：是否有实质的「设计目标」（非空、非套话、明显来自讨论结论）；是否混入代码实现内容（类名/方法名/API 表/文件清单/执行流程伪代码）——有则标 ⚠️。**不**检查"公开 API 是否与源码一致"——SPEC 不再承载实现索引，结构问题查 CodeGraph
3. **报告结果**：

| Spec 文件 | 状态 | 问题 |
|---|---|---|
| agent/SPEC.md | ⚠️ 需纯化 | 混入大量实现索引（架构图/类清单/执行流程），设计目标被淹没 |
