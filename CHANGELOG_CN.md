...
## v10.39.0 (2026-05-10)（最新版本）

> 本版本首页新增关切总览仪表盘，所有定时任务状态一目了然；对话中图表可交互拖拽，不再是静态截图；自动任务失败不再静默，主动通知到位；支持对接更多外部服务。

### 新功能
- 📈 **首页新增关切总览仪表盘**：关切总览面板二期优化 + 状态汇总条 + 欢迎页入口，4 卡片改 grid 布局
- 📊 **对话气泡活图**：Agent 生成的 ECharts 图表可在对话气泡中直接渲染，支持鼠标悬浮交互，告别图片截图；chart skill 投递 echartsOption 载荷；修复活图大图打不开及缩略图视觉一致性问题
- 🌐 **MCP Streamable HTTP**：支持 Streamable HTTP transport，修复 SSE headers 未生效，加入 JSON 一键导入；修复导入弹窗空白及已连接服务器边框样式
- 🤖 **独立助手示例展示**：新增独立助手能力示例网格与闪烁提示功能；卡片瘦身 + 所有示例 prompt 确定可执行
- 🎬 **流式预卡片**：skill_create/skill_update 流式输出时预卡片显示内容进度，不用干等

### 改进
- 🔧 **子 Agent 重构**：抛弃 Fork 模式，恢复独立模式，类型精简至 read/write；dispatch_agents 提示词明确子 Agent 不可用 MCP 工具
- 🔧 **i18n 重构**：按业务域拆分中英文翻译文件 + 加入对齐检查脚本；补上遗漏的中文场景文案
- 🔧 **凭证安全重构**：用 Electron safeStorage 替代 keytar，启动后凭据弹窗从 N+1 次降到 1 次
- 🔧 **chart 数据校验**：校验错误信息附带 received data 顶层 keys，排查更容易
- 🔧 **关切觉醒打通**：关切执行失败作为生命周期事件接入觉醒机制，不再静默消失
- 🔧 **微信适配器常量规范**：变量声明从 let 改为 const
- 🔧 **K 线副图打磨**：副图视觉打磨 + formatter marker 协议安全闸门 + 预览导航活图还原
- 🔧 **MCP 配置管理**：config-skill 补充 MCP 服务器管理相关描述

### 问题修复
- 🐛 **Agent 全局历史污染**：新开 tab 第一次对话不再被全局历史污染
- 🐛 **web_fetch 上限偏低**：默认大小上限 1MB → 3MB，硬上限 5MB → 10MB
- 🐛 **语音回退**：回退 AudioWorkletNode 重构 + 补 macOS 麦克风权限声明
- 🐛 **导航标题一致**：顶部导航"指南"改为"使用指南"，与其他标题风格一致

### 文档
- 📝 **17 个核心 Service SPEC**：为 17 个核心 service 补 SPEC + 全景索引 + 强制规则
