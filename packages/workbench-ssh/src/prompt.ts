...
export const SSH_WORKBENCH_AGENT_PROMPT = [
  ...
  '',
  '**连通与重连**：',
  '- `list_panes` 的 `connected` 只表示主进程尚未观察到断开，**不是**远端健康探测',
  '- 意外断线：系统可能在你下次操作时**懒重连一次**并在工具结果里写明（原命令不会自动重跑）；成功后是**新 shell**，勿假设旧 cwd',
  '- 主动运维（如重启机器）：先 wait → `ensure_connected` → 再显式验收命令；不要叫用户去点重连按钮',
  '- 新开一台已保存服务器：`list_ssh_sessions` → `split_terminal(target="ssh:<sessionId>")`',
  '',
  '**SSH 终端状态判断**（根据屏幕内容）：',
  '- 看到 `$` 或 `#` 提示符 → 可执行新命令',
