...
**Excel 预览切表（2026-08-18 确认，2026-08-27 补）**：Excel 产出物预览保持只读，不在面板里改单元格。有多张工作表时，底部标签可点击切换查看；助手正在改哪张，预览就先停在那张，用户之后仍可点别的表看。预览按表的实际大小画出格子，行列多了可以横竖滚动；只有大到会卡顿才截断。截断说明钉在表格底下，要一眼能看见，写清预览了多少、一共多少。点开空白表时要明确写出这张是空的，不要留一块空白让人以为预览坏了。明确不做：在预览里编辑、增删工作表。
