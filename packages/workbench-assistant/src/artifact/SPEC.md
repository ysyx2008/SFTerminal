...
**文档铺满（2026-08-25 确认）**：文件默认仍是右边参照。读长文、改一章、看宽表时，人可以把这份文件临时变成工作面——文档占满工作区，对话流藏起，底下留下输入，文档上浮一句它刚说的或它在忙什么。要你确认、回答、输密码时，这句话让开，完整出现。这不是第三种座位，只是文件在座时多一种看法。

- **默认分栏**：短回复、几行稿子不自动铺满。不按类型、字数、标题猜测。
- **人点才进**：文档顶栏一颗布局按钮，分栏和铺满来回切。助手新写出东西：已经在铺满就留在铺满并换成这份；在分栏就还在分栏。
- **只在这一次入座期间记住**：换页签、点清单换文件，保持铺满。收起或关光页签再打开，回到分栏。终端入座时铺满结束。不写进历史，不跨会话。
- **浮层就是那句话**：没有「最近一条」标题，不贴文件卡片，不点赞。点卡片只展开或收起这句，不冲回分栏。回分栏只走顶栏按钮。还没说过话、也不在忙，浮层不出现。
- **输入垫在文档底下**：占一条高度，不叠在正文上。划选改文档、排队、过程那一行的口径都不改。
- **明确不做**：系统全屏、藏侧栏；为铺满另写一句漂亮人话；把终端也改成一句浮层；铺满偏好写进设置。
