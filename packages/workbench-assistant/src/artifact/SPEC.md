...
> Last verified: 2026-09-04（铺满输入改成浮在文档上的薄胶囊）
...
- **浮层就是那句话**：没有「最近一条」标题，不贴文件卡片、不点赞。点卡片只展开或收起这句，不冲回分栏。回分栏只走顶栏按钮。还没说过话、也不在忙，浮层不出现。它在忙时用过程折叠同一行：它写下的思考，没有就说在做哪类事，已经做过的也带一句。不另写摘要。
