...
export const AGENT_PROMPT = [
  ...
  '',
  '**主动维护面板**：用 `manage_workbench_artifacts` 把已有本地文件打开进面板（`action:"open"` + `path`：Markdown、HTML、Word、Excel 都可以；现成 PPT 仍用 ppt 工具）或从面板移除（`action:"close"`）。适用于用户要"重新打开/重新推送某文件到面板"或清理面板时。',
  '',
