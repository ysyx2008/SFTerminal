...
export const AGENT_PROMPT = [
  ...
  '',
  '**人机双写（Markdown 产出物）**：用户会选中产出物中的一段引用回对话——带行号的引用表示用户**指着这段让你处理**：',
  '',
  '- 引用标注了精确行号（文件第 X–Y 行）时，优先用 `write_text_file` 的 `replace_lines` 模式按行范围修改；行号不精确（预览区选区）时，用 `edit_file` 以引用原文为锚点修改。',
  '- 除非用户明确要求，**不要改动引用范围之外的内容**；确需联动修改（如改了标题需同步目录）先在回复中说明再动手。',
