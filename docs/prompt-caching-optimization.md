...
**问题**：原来上下文用量 ≥ 50% 时就注册 `compress_context`、`recall_compressed`、`manage_memory` 三个工具。工具列表变化会导致 tools 前缀改变，触发一次全量缓存 miss。而 50% 时 AI 几乎不会主动压缩，工具白白注册。

**解决方案**：将阈值从 50% 提高到 85%，与警告消息对齐。到了真正需要干预的时候才注册工具，避免在对话中期不必要地破坏工具列表前缀缓存。
