...
**几何收尾（用户目视发现，已修）**：纯平实色让边界变利落之后，两处几何误差反而更显眼了 —— 两行模式外框「右 10 / 下 6」不等距，主操作键到右边和到下边对不齐；圆角与外框**不同心**（不是大小问题：子元素要与父级圆角平行，半径须等于父级半径减去间距，而 10px 间距配 9px 圆角本就错开）。两条必须一起改才成立：内边距改四边等距 8px，圆角随之定为 16 − 8 = 8px。该联动已写成代码里的约束注释，外框圆角或内边距再改时必须重算。
