...
>
> **唯一要保留的机械信号**：汇报机制盖不住「汇报者本身失效」——关切 timeout 意味着 Agent 没跑完，压不到 `talk_to_user` 那一步；崩掉的关切无法自己报崩。该洞已由侧栏关切入口的常驻异常角标堵上（见下方取舍 G），不需要在首页再来一份。
>
...
**此项不随取舍 A/D 回滚** —— 它独立成立：即使首页卡片回来了，常驻可见的异常角标仍是净收益（卡片角标只在首页看得见）。代价是关切暂时有两个入口，属用户明确接受的「删减重构以后再说」范围。
