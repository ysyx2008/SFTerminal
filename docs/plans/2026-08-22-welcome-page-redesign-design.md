...
- [x] **步骤 2 — 输入框成为主体** ✅ 部分完成（描边未动）
      实际做的是**加高**：欢迎页文字区最小高度提到 56px（整框约 74 → 110px）。这一屏只剩输入框一个主体时，一行高的框读起来像搜索框。只在欢迎页放大，侧边面板纵向空间紧张不跟着变。用 `min-height`（约束，赢过测量逻辑写入的行内高度，且不妨碍继续长）而非改高度。
      原计划的「常态低对比边框、`:focus-within` 才上强调色」**未做** —— 卡片恢复后输入框不再是唯一主体，这条的前提变了，留待信息架构重做时一并判断。
