...
>
> **唯一要保留的机械信号**：汇报机制盖不住「汇报者本身失效」——关切 timeout 意味着 Agent 没跑完，压不到 `talk_to_user` 那一步；崩掉的关切无法自己报崩。曾打算由侧栏关切入口的常驻异常角标堵上，但取舍 G 已回退（见下），**此洞目前敞着**：异常只在秘书菜单和首页那张卡上看得见，两处都要用户先走过去看。重做时先还这笔。
>
...
>
> **代价要记住**：异常角标退回秘书头像菜单里，菜单不打开就看不到 —— 有异常不会主动告诉你。首页那张卡的角标只在首页可见。也就是说「一眼看到有没有出问题」这条产品要求目前仍未被满足，将来重做时这是第一个要还的债。
