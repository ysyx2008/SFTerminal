...
- 日期：2026-08-22
- 状态：**未完成**（视觉精致化已完成；**信息架构部分整体暂缓** —— 详见 §2 取舍 A/B/C/D 上的推翻记录）
- **2026-08-23 结论**：视觉层全部保留；信息架构层（撤重复入口、新建注意力区块）**试过又回滚**，用户决定「结构先保持原样，删减重构以后再说」。原因不是做不出来，是讨论中发现方向本身有误，见 §2 取舍 B 的推翻记录。
...
### 取舍 A：删掉「快速开始」，但按构建分叉（**已实施 → 2026-08-23 回滚，暂缓**）

> 回滚记录：完整版撤掉后目视确认「层次终于对了」——视线从四个彩色卡片图标转回输入框，主操作第一次排在视觉第一位。但撤完之后暴露出更根本的问题：这一屏**对谁都长一样**，撤掉重复内容等于从「放错了东西」变成「什么都没放」。用户决定先恢复原结构，把删减留到有独有内容可以填进去之后再做。下方原文保留，作为将来重做时的依据。
...
### 取舍 B：下半屏放「需要你注意的」（**2026-08-23 推翻，未实施**）

> **推翻理由（比回滚更要紧，是方向错了）**：这个区块打算聚合关切异常、失败会话、逾期待办、未读联络等原始信号，自己拼出「你该注意什么」。但架构里**已经有一条官方通道做同一件事**：关切与唤醒的执行都是内心独白，要跟用户说话必须经 `talk_to_user` 冒泡进**联络**，且有 30 分钟 TTL 的主动消息暂存专门衔接上下文。
>
> 也就是说这个区块是在给同一个概念造第二条通道，而且是更差的那条——它读机器原始状态（`lastRun === 'failed'`）、用模板句表达；联络读的是**秘书自己的判断**、用秘书自己的话表达。项目规则明确禁止给一个概念造第二个真相源，这条正好撞上。
>
> **新方向**：下半屏放联络里秘书最近对你说的话（一段引文 + 一键接着聊），数据走既有的联络合并视图，不新建判定。
>
> **唯一要保留的机械信号**：汇报机制盖不住「汇报者本身失效」——关切 timeout 意味着 Agent 没跑完，压不到 `talk_to_user` 那一步；崩掉的关切无法自己报崩。曾打算由侧栏关切入口的常驻异常角标堵上，但取舍 G 已回退（见下）。

> **回退后此洞剩多大（2026-08-23 核实，修正先前「此洞敞着」的说法）**：首页关切卡右上角本来就有异常角标，且随每次关切执行完成实时刷新；首页 `v-show` 常驻挂载，数字不会过期；冷启动无会话在焦点上，落地屏就是首页。所以**开机一眼能看到**这条是成立的，不需要在首页再造一份。
>
> 真正剩下的只是一个时间窗：用户在会话 / 终端里干活时首页被盖住，那期间发生的异常要等他退回首页才看见 —— 是「要走过去看」，不是「看不到」。要不要堵这个窗，等有真实抱怨再说。
>
> 下方原文保留，其中「只读既有真相源、`aborted` 不算异常」等约束对新方向仍然成立。
...
### 取舍 D：「最近连接」撤出首页（**已实施 → 2026-08-23 回滚，暂缓**）
...
### 取舍 G：关切提到侧栏固定入口（**已实施 → 2026-08-23 回退**）

> 回退记录：与取舍 A/D 一并回到改动前状态，侧栏与主窗口两个文件精确还原，无残留。**下方那条「此项不随 A/D 回滚」的判断被推翻了** —— 卡片恢复后关切有了两个入口，用户选择结构一律维持原样，把这一项也归入「删减重构以后再说」。
>
> **代价（2026-08-23 核实后修正，先前写的「仍未被满足」是夸大了）**：秘书菜单里那份角标确实要打开菜单才看得见，但**首页关切卡自带实时角标，且冷启动的落地屏就是首页** —— 「一眼看到有没有出问题」这条在开机场景下是满足的。回退真正丢掉的只是「在会话 / 终端里干活时也能看见」这一段，见取舍 B 的核实附注。

原以为撤掉首页卡片会让关切失去入口，核实后发现秘书头像菜单里本来就有一项。但这恰好暴露了真问题：**关切唯一看得见的入口是首页那张卡**，菜单里那项要用户先猜到「点自己的秘书头像会出现功能菜单」。

更严重的是角标：异常数挂在那个菜单项上，菜单不打开就看不到 —— **有异常也不会告诉你**。这比入口深得多的问题。

做法：关切与新对话 / 联络 / 终端并列进侧栏固定入口，异常数常驻可见；同时**从秘书菜单移除**（留两个入口就是给同一件事造两处真相，日后必然各自演化）。待办暂不动。

两处必须同时清理的连带残留：`pick()` 的 `open-watch` 分支（已无调用方），以及头像提示点里的关切异常项 —— 关切搬走后它会变成「头像有点、点开菜单里却没这件事」。

面板形态与邻居不同（全屏覆盖层，不是一个「地方」），自己推不出选中态，会是这排里唯一永不高亮的项：由主窗口把面板开合状态传进侧栏解决，顺带补上「面板盖住全屏时界面没有位置指示」这个既有缺陷。

~~**此项不随取舍 A/D 回滚**~~ —— 已于 2026-08-23 一并回退，理由见本节开头。上面「常驻可见的异常角标是净收益」这个判断本身没被否掉，被否掉的是「现在就单独留着它」：卡片回来后两个入口并存，与结构一律维持原样的决定冲突，因此归入同一批暂缓。
...
.welcome-title {
  /* 2026-08-23 修正：先降到 22px/600，目视偏弱 —— 显得廉价的是渐变不是字号，
     两个一起砍把底气也砍掉了。渐变仍然不要，字号回到 26px/700。 */
  font-size: 26px;
  font-weight: 700;
...
- [ ] ~~**步骤 2 — 输入框成为唯一焦点**~~ **前提已消失，暂缓**
      试过**加高**（文字区最小高度 56px，整框约 74 → 110px）：下半屏空着时成立，卡片恢复后**已撤销** —— 加高会让输入框比卡片还高、成为整屏最高的单个元素，比例就反了；它该是一条清瘦的横条，让卡片比它高。
      原计划的「常态低对比边框、`:focus-within` 才上强调色」同样未做：这一步整体建立在「输入框是唯一主体」之上，而卡片回来后它不是了。留待信息架构重做时一并判断。
      *顺带定下的一条与主体之争无关、独立成立*：内容区左右内边距 20 → 40px。原值只在面板宽度远大于 760px 上限时够用；侧栏展开后面板约等于上限宽度，20px 就是全部边距，内容顶着边缘。40px 在宽窗口下空转（居中留白本就更宽），只在顶着上限时收进来。

- [ ] ~~**步骤 3 — 移除重复入口**~~ **已实施 → 2026-08-23 回滚（见取舍 A/D）**。回滚后完整版的 `.quick-start` 与「最近连接」判断条件与改动前完全一致；i18n 键与 emit 未清理（撤销后仍在使用）。
...
- [ ] ~~**步骤 4 — 「需要你注意的」区块**~~ **方向已推翻，不按原文实施（见取舍 B）**。取而代之的方向：下半屏放联络里秘书最近说的话，走既有联络合并视图。此步暂缓，与取舍 A/D 一起等「有独有内容可填」时重做。
      *若将来实施，验收仍需人为造条目看，这类东西单测测不出来。*
