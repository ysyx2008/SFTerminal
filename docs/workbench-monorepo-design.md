...
> 起因：业务条线研发团队需自行研发并发布专用工作台，每个工作台可一站式包含前端 UI + 专用技能 + MCP
...
- **消费者是谁**：业务条线研发团队（金融、运维、人事等），通过 AI 辅助编码（TS + Vue3 + Pinia）开发专用工作台
- **业务工作台需要什么**：① 自己的前端 UI；② 声明依赖哪些 skill；③ 声明自带哪些 MCP server；④ 一段给 Agent 的 system prompt 片段
- **业务工作台不需要什么**：碰 `Agent` 类内部、`Conversation` 聚合根、`useAgentMode` 实现、后端服务源码

判据：**业务工作台不需要的，就不进 SDK**。判据反向也成立：core 里 desktop 应用层胶水（ai-debug / menu / xshell-import / skill-market 等）留在 `apps/desktop`，不抽进任何包。
