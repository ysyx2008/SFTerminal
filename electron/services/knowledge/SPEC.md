...
### 备份和带走是同一件事（2026-08-29）

- **问题**：设置里并排放着「导出 / 导入」和「备份 / 恢复」，人以为是两套。其实都是留一份、再盖回去。导出还不带记忆，导入是整库硬盖、也不补缺的索引。
- **成功标准**：设置里只剩备份和恢复。本机留份、自动留份、存到别处、从文件夹盖回来，都是同一份快照，带走的是整包（含记忆）。从别处盖回来和从本机列表盖回来一样：先把当前这份挪开，再补缺的索引。
- **明确不做**：不再单独放导出 / 导入。不为以前「导出」出来的旧文件夹另做一套恢复——那种拷贝不完整。
