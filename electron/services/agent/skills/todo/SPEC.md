 ...
 *
 * - **预防优于追责**：即将到期单独成区并置于活跃列表最前（含：本地日历「今天」截止，或剩余时间占比 ≤ 20%），视觉热度 ≥ 已逾期；催在逾期前。分区互斥：符合条件的活跃项只出现在「即将到期」，不重复出现在进行中/待办
 * - **双通道不抢戏**：
 *   - **标题色** = 综合关注度：重要度（`priority`）× 时限档 × 陈旧度（`createdAt`，轻量补洞）；「今天」主要靠分区 + 截止芯片警示色突出，不为「剩 N 天」再拆多档颜色
 ...
 * - **颜色（跟主题走）**：今天分区/芯片/`strong|critical` 标题用 `--brand-alert`；已逾期分区/芯片与高优/`medium|mild` 标题用 `--accent-warning`；普通标题 `--text-primary`；禁止硬编码 hex
