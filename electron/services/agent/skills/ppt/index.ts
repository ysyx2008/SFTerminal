...
const pptSkillContent = [
  ...
  '',
  '### 画布与定位',
  '- 16:9 画布 = 1280 × 720 px（4:3 为 960 × 720）。用绝对定位：position:absolute; left/top/width 单位 px。',
  '- 安全边距：四周留约 64px；底部留白 ≥ 48px（否则报溢出错误）。',
  '',
  '### 硬性规则（违反会报错并要求你改）',
  '1. 文字必须放进 p / h1-h6 / ul / ol；div 里不能有裸文本。',
  '2. 文本标签不能带 background/border/shadow。卡片、色块用 div，文字放在 div 内的 p / h*。',
  ...
  '',
  '两栏要点（左右各一张 card，卡内放 h3 + ul）：左栏 left:80;width:540，右栏 left:660;width:540。',
  '',
  '### 长 PPT 分批生成（防输出截断）',
