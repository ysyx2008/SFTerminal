...
产出物里的表格预览按表的实际大小画，人可以横竖滚动看完整内容。只有大到预览会卡顿的表才截断；截断说明钉在表格底下，要一眼能看见，写清预览了多少、一共多少。点开一张空白表时，要一眼看出是空的，不要像预览坏了。预览里可以圈一块格子，让助手只改这块，改完立刻在预览里看见；发出去之后高亮还在，方便对照。圈完直接去输入框写要求，不要悬浮提示，也不要右键快捷菜单。不在格子里直接打字。格子上的字体、字号、颜色和底色要跟着表走，一眼能对上。有明确颜色的照画；主题色按常见默认色板近似，工作簿自定义主题不做精确还原。写成文字的公式、或按文本格式存的公式，预览里要显示公式原文，不要换成算出来的数；真正在算的公式才显示值。条件格式、图表、图片不做。助手通过读取拿到的内容仍然受上下文限制，不跟预览一起变大。
