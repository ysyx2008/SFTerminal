...
- candlestick:    { categories: string[], values: [[open, close, low, high], ...], volumes?: number[] }
          ...
          width: {
            type: 'number',
            description: '画布宽度（px），默认 1280，上限 7680。**应根据数据规模主动设定**：数据点 ≤ 10 用 800-1280；10-50 用 1600-2400；50-200 用 2400-3840；200+（如半年/全年日 K、全天分时图）用 3840-7680。时间序列长 → 宽高比拉宽（≥16:7）。字号会随宽度自动缩放（基准 800px=1.0×，1600=1.4×，3200+=2.0× 上限），所以放大画布时字号也跟着变大、不会显小，可以放心容纳更多数据。'
          },
          height: {
            type: 'number',
