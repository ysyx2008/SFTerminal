...
> Last verified: 2026-08-31（联络干活时：用户可插话，关切找人要等这场结束）
...
>   3. 这场结束后自动开的下一件（排队里接着干的），不要当成在回那条提醒，也不要带上「我刚找过你」。你下一句亲口说的、或聊天软件回的，才带上。
> - **关键取舍**：能发，不等于能打断。用户插话是这场的一部分；关切找人是另一路，要等这场完。自动开的下一件是接着干，不是在回提醒——排队时提醒还没上墙，看不见也就谈不上在回。
> - **明确不做**：不做成关切的话中途插进正在跑的这场；不因此让用户打字也等结束；不靠比对排队和提醒谁先谁后来猜。
>
