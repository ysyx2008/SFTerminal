...
> Last verified: 2026-08-18（助手开真终端：换台 / 滑回）
...
> - **关键取舍**：文件继续往右开；终端是换台，不是往右开。开了终端后右边只留给对话，已打开的文件先收进清单，第一期不跟终端同时抢主位。终端页本身不用改——那边页还是终端，关页仍然散场。
> - **明确不做**：三栏同时铺开；把终端当成产出物预览；把助手会话改成终端页（形态不变）；关切 / 远程渠道 / 纯命令行里强行开这种看得见的台。
