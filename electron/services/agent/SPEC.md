...
> Last verified: 2026-08-21（联络各渠道用界面上选的模型）
...
> **远程会话也能读写本机文件（设计目标，2026-08-21）**：
>
> - **问题**：连着远程服务器时，用户希望顺手在本机写文档、记笔记，但 Agent 说做不到——本机读写被关掉了。
> - **成功标准**：
>   1. 远程终端这场对话里，Agent 能读、写、改、搜索本机上的文件。
>   2. 这些能力的说明只陈述工具本身做什么，不另加「别跟远程搞混」之类的操作指导。
>   3. 相对路径按本机目录理解：当前窗格是本机终端的，仍跟那个终端的当前目录走；当前窗格是远程的（或没有本机终端），落到本机用户主目录。绝不拿远程当前目录往本机上拼。
>   4. 远程文件仍走原来的远程写入和上传下载，不受影响。
> - **关键取舍**：连着远程不等于不能碰本机。工具清单如实给出本机读写；怎么选由模型自己判断。
> - **明确不做**：不因此开放并行子任务、做幻灯片等其它仅本机的能力；不在提示词里教模型该用本机还是远程。
