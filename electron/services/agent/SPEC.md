...
> Last verified: 2026-08-27（用户插话后折叠行要停表）
...
>   2. 选中的立刻装上，用胶囊挂在输入区附近（不混进这一条要发出去的文件），整场对话一直在。
>   3. 秘书自己装上的，也出现在同一排胶囊里。
>   4. 点掉胶囊就卸掉；这场对话里它不能自己再装回来。用户再 @ 一次才重新开。
>   5. 新开一场是空的。重开这场时，当时还开着的会挂回来；已经卸掉的不要回来。
> - **关键取舍**：胶囊就是「现在开着」，不是给模型看的一行字。只在新开上的那一下轻轻亮一下，不要做成抽卡。
> - **明确不做**：外部工具包不进这张菜单、也不进这排胶囊。不在每条消息里反复闪。关切和唤醒不走这套界面。
>
