...
>   5. 关切、唤醒自己跑完不叠这几声，它们已经有自己的通知。
>   6. 默认开。设置里能关，也能换成自己的音频，还能改回默认。
>   7. 默认那几声必须好听、短、不刺耳；宁可偏轻，也不能做成让人想关的蜂鸣。
> - **关键取舍**：先靠短音效，不靠念一段语音。语音要配服务、会晚半拍，还容易每次都念「任务完成」让人烦。想用人声，就换成自己的一段录音。
> - **明确不做**：不搬现成聊天软件的版权提示音；不把回复整段朗读当成完成提示（那是另一件事，要另外开）；不为关切/唤醒再叠一层桌面提示音。联络回消息、主动找人先不单独给一声——和任务完成不好区分，等以后有更合适的再说。
