...
> Last verified: 2026-09-04（同一场对话先带原文，窗口满了才交接）

## 职责

AI Agent 的核心执行引擎。接收用户自然语言指令，通过 ReAct 循环（思考→工具调用→观察）自主完成任务。支持本地终端、SSH 远程、无终端助手三种模式。

> **同一场对话先带原文，窗口满了才交接（设计目标，2026-09-04）**：
>
> - **问题**：长对话加载历史后再续聊，它会忘掉前面说过的话。人还看得见整场，它却去翻历史、搜文件。根因是两套事揉在一起：按远近把过程甚至问答收成提要，和窗口真满时的交接，被当成同一套「压缩」。
> - **成功标准**：
>   1. 同一场里带着原文：你说过的话、中间怎么做的、这一轮实际怎么收场的。正常做完就留最后那句交代；被你停掉、做失败、或还在等你拍板，也照实留下。没有正常总结，也不许整轮消失，更不许编一句成功的收场。不因为隔得远就把过程先收掉。
...
> - **关键取舍**：记得「这场都说了什么」靠原文。整场已经塞满，靠交接。交接是检查点，重开接着用，不把压掉的再展开。两件事，不另做一套按远近收过程的档位。
> - **明确不做**：不用提要或砍首句来代替同一场里的原文。不为了省地方按远近把中间过程先收掉。不为唤醒「扫很多场只要一眼」改同一场续聊。不另做一套交接——已有的那套继续用，但必须跟对话一起记住。不为了省预算再按比例克扣原文该占的位置。不因为这一轮没有正常总结就丢掉你的原话，也不补一段它没说过的收场。不把已经交过的原文在重开时重新灌回去。
>
...
> **冷启动带原文，有检查点就接着检查点（2026-08-20；2026-09-04 改定）**：
>
> - **问题**：重新打开一场对话再续聊，模型只记得最近一轮，再往前只剩一句摘要。人看到的是同一场对话，模型却像没看过前面写过的东西。
> - **成功标准**：还没做过交接时，冷启动把这场原文原样装回去。离开前若已有交接检查点，接着用检查点，不把已经交过的原文再展开。
> - **关键取舍**：原料是这场对话本身（发给模型的那些话），不是界面上的执行步骤。加载历史后步骤可能是空的，不得因此丢掉对话。
> - **明确不做**：不按远近把过程先收成要点或清单。不用提要或砍首句代替原文。不把已经交过的原文在重开时重新灌回去。
...
> **装载量与预算**：`restoreRecentTaskMemory` 按 kind 分流装载——companion 同一场续聊，条数不是上限（装多少由窗口和交接决定）；wakeup/watch 仍放宽到 20 条记录 / 30 个任务（配合唤醒的广度优先 + 一句话概要）。仅防内存膨胀；真正进上下文的量由窗口预算与唤醒的 `maxTasks` 裁剪。
