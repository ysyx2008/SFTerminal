...
> **任务可以先锁在规划里（设计目标，2026-08-22）**：
>
> - **问题**：用户想先想清楚再动手，但现在没有一把真正的锁——它仍可能改文件、跑命令、发邮件。现有的步骤清单只是执行进度，挡不住误动手。
> - **成功标准**：
>   1. 只有任务会话有这把开关，默认关。联络、关切、唤醒没有。
>   2. 只有用户能拨开、拨关。对话里说「开始做」不算放行。
>   3. 开着时：可以探环境（读、搜、看日历邮件网页、跑不会改世界的查看），可以往它自己的草稿区写东西；不能改用户的世界（写用户文件、改系统、发信、建日程、建关切、对别人说话等）。
>   4. 关着时：恢复现在的能力。拨关本身不自动开干，等用户再说一句才动手。
>   5. 开关跟着这场任务走：换页、重开、继续聊，还是上次拨的那个状态。
>   6. 不规定它必须交某种方案文档，也不规定格式。它觉得该落草稿就落，不落也可以。
>   7. 这把锁必须是硬的：不能靠它自觉。环境里此刻做不到的事，就不该出现在它面前；即便它硬调，也必须被拦住。
> - **关键取舍**：宁可多挡一次、等用户再说一句，也不要「我只是想先解锁看看」结果已经开始改东西。规划锁和执行清单是两件事，清单继续只表示进度。
> - **明确不做**：不因此新增第五类会话；不给联络/关切/唤醒做这把开关；不规定方案必须长什么样；不把聊天里的「开始做」当成关锁。
...
>   8. 提示音有总开关，四声各有开关。总开关关掉，四声都关；再打开，四声都开——不记住关掉之前各自的状态。总开关开着时，仍可单独关某一声。默认都开。也能换成自己的音频，还能改回默认。四声共用一档音量，设置里可调，也可以调得比默认更响。
...
- 不废弃按规则降级那套。它退到两个位置：AI 压缩失败时的兜底，以及重启后从记录恢复上下文——后者没有替代品
...
> - **成功标准**：它说给用户听的话永远在外面，它做事的过程永远收着一行；那行从头到尾位置不动，跑着时说在忙什么并走秒，做完说做了什么、花了多久；过程中某次工具失败收进去，只有整件事没办成才告诉用户；要用户动手的（确认、提问、密码）永远在外面；任何任务都收，形态始终一致；点开能看到底稿，也可以在设置里整个关掉。
