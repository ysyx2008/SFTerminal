...
>   4. 点掉胶囊就卸掉；秘书不能自己悄悄再装回来（重开按清单恢复、预加载也算悄悄）。用户再 @ 一次可以；秘书自己调 load_skill / load_user_skill 装上的，也允许。
>   5. 新开一场是空的。重开这场时，当时还开着的会挂回来；已经卸掉的不要回来。
>   6. 通用 → 界面里可以关掉这一排胶囊。关掉只是不看见，装上、卸掉、记住、重开都照常。
> - **关键取舍**：胶囊就是「现在开着」，不是给模型看的一行字。新开上的那一下要让人觉得秘书刚变强了，不要闪光抽卡。不想看见这一排时，关掉即可，不要因此卸掉技能。
