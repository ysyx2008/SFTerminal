...
> Last verified: 2026-08-26（提问标出最推荐的选项）

## 职责

AI Agent 的核心执行引擎。接收用户自然语言指令，通过 ReAct 循环（思考→工具调用→观察）自主完成任务。支持本地终端、SSH 远程、无终端助手三种模式。

> **提问必须带上推荐选项（设计目标，2026-08-26）**：
>
> - **问题**：向用户提问时经常只丢一句开放问题，用户得自己打字。明明可以点选的推荐答案没有给出来。给了选项也全长一个样，用户还得自己比较。
> - **成功标准**：
>   1. 每次提问都带上至少两个推荐选项，用户点一下就能答。
>   2. 选项里标出最推荐的那一个，用户不用自己比较。没人答时也用这一个。
>   3. 用户仍可以自己打字，不被迫只能选给的那些。
>   4. 没带够选项、或没标明最推荐是哪一个的提问，不会真的问出去，会让 Agent 补上再问。
> - **关键取舍**：点选是默认答法，自己打字是退路，不是反过来。最推荐的必须是选项里的某一个，不另写一句。
> - **明确不做**：不自动补一个「其他」选项；不靠回车代选推荐项；不因此改超时或无人值守时的规则。
>
