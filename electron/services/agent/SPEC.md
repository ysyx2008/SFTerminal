...
> **助手开真终端（设计目标，2026-08-18）**：
>
> - **问题**：独立助手只能在幕后执行命令，用户看不见；若把终端塞进右侧产出物，又不像终端用户习惯的「左边干活、右边 AI」。旗鱼已经有完全透明的真终端（命令和结果都打在窗里），缺的是让助手自己决定连哪一台、连上后换到熟悉的终端台。
> - **成功标准**：
>   1. 助手可以自己开一台真终端：本机，或通讯录里已保存的远程。开出来的就是现有那套能干活的透明终端，不是预览、不是幕后执行。
>   2. **开第一扇 = 换台**：同一场对话还在，工作台换成终端台——左边是终端，右边是原来这场对话（从主位滑到辅助位）。
>   3. **关最后一扇 = 滑回助手台**：管子拆掉，对话和 Agent 都还在。空着的终端台不留。关的是工作面，不是这场任务。用户也可以自己在终端栏顶栏关掉，效果一样。
>   4. **关掉这场对话**才带走 Agent（和现在一样）。
>   5. 一旦开了真终端，命令必须打在看得见的那扇窗里，禁止一边开着终端一边在幕后默默执行。
>   6. 窗格仍用现有的「列清单 / 再开一扇 / 关掉 / 切焦点 / 确保连通」；新增「不分屏、直接连」——助手没有第一扇时用它开第一扇。
>   7. 关窗时若 Agent 正在那扇窗里跑命令，必须告诉它窗已经没了，不能假装还能打进去。
>   8. **用户不能从顶栏再开第二个标签**（2026-08-19）：一个标签对应一个 Agent，这场对话只有一个 Agent。分屏开多扇窗可以留（还是同一个 Agent），但不要让用户像终端页那样再加一个标签。
> - **关键取舍**：文件继续往右开；终端是换台，不是往右开。开了终端后右边只留给对话，已打开的文件先收进清单，第一期不跟终端同时抢主位。终端页本身不用改——那边页还是终端，关页仍然散场，也仍然可以多开标签。
> - **明确不做**：三栏同时铺开；把终端当成产出物预览；把助手会话改成终端页（形态不变）；关切 / 远程渠道 / 纯命令行里强行开这种看得见的台；在独立助手的终端台上给用户「再开一个标签」。
...
> - **成功标准**：点停止后，还没开始的工具不再启动；已经在跑的工具尽快停下来，卡片变成已中止，会话结束。
