...
> - **成功标准**：开了「自动切换可用模型」时，当前这款按自动重试仍失败，就按配置列表顺序换下一个能用的；这场对话的模型选择器跟着变，对话里说清楚从哪个换到了哪个。
...
> **等后台命令要看得见（设计目标，2026-08-20；同日修订）**：
>
> - **问题**：长命令转后台后再去等结果时，界面只剩一个任务编号，人看不出在等哪条命令。
> - **成功标准**：等后台任务时必须能看见正在跑的那条命令，不能只剩一个编号。
> - **明确不做**：不给每条普通命令额外挂「已运行多久」和一段输出摘要——短命令本来就有结果，再贴这些是打扰。不为此另开一扇终端；不等待过程中向模型实时喂输出。
