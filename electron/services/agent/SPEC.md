...
> Last verified: 2026-08-26（提问推荐不强制）
...
> - **明确不做**：不靠解析内容来决定要不要上墙。
>
> **提问可以带选项，推荐不强制（设计目标，2026-08-26）**：
>
> - **问题**：向用户提问时经常只丢一句开放问题，用户得自己打字。给了选项也全长一个样。但有时确实没有更倾向的那一个，不该逼它硬标，说明也不该写成命令。
> - **成功标准**：
>   1. 提问可以带上供点选的选项，用户点一下就能答；不带选项也能问出去。
>   2. 有更倾向的那一个时标出来，没人答时也用它；没有就不标，提问照样出去。
>   3. 用户仍可以自己打字。
>   4. 给它看的说明只陈述这件事能怎么用，不命令它必须推荐。
> - **关键取舍**：建议有选项、建议标倾向，都不强制。没推荐也能问。
