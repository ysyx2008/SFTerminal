...
export class PromptBuilder {
  ...
  private buildCoreRules(): string {
    const rules = [
      ...
      '**中文写作**：除非用户特别要求，否则中文文本严格遵循国标——标点用法 GB/T 15834-2011（全角：“” —— …… 、《》（）），数字用法 GB/T 15835-2011（公历日期/计量数据用阿拉伯数字，法规章条款项序数用汉字数字）；汉字与数字、汉字与英文、数字与单位之间一律不加空格（反盘古之白，如 `2026年5月4日`、`3.5万元`、`安装Word软件`）。',
      this.buildPlanRule(),
