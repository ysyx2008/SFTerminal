...
[系统/任务的当前状态]`,
    ...
    'tool_output.externalized_head': '[完整输出共 {total} 字符，已保存到: {path}\n以下为开头摘录；需要完整内容请用 read_file 对该文件分段读取（start_line/end_line）。]',
    'tool_output.externalized_tail': '[完整输出共 {total} 字符，已保存到: {path}\n以下为末尾摘录；需要完整内容请用 read_file 对该文件分段读取（start_line/end_line）。]',
    'tool_output.externalized_only': '[完整输出共 {total} 字符，已保存到: {path}\n当前上下文余量不足，未附摘录；请用 read_file 对该文件分段读取（start_line/end_line）。]',
...
[Current state of system/tasks]`,
    ...
    'tool_output.externalized_head': '[Full output ({total} chars) saved to: {path}\nHead excerpt below; use read_file with start_line/end_line to read it in parts.]',
    'tool_output.externalized_tail': '[Full output ({total} chars) saved to: {path}\nTail excerpt below; use read_file with start_line/end_line to read it in parts.]',
    'tool_output.externalized_only': '[Full output ({total} chars) saved to: {path}\nContext headroom exhausted, no excerpt attached; use read_file with start_line/end_line to read it in parts.]',
