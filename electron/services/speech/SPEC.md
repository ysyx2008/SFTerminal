...
- 设置 → AI：「语音识别模型」：状态行 + 动作按钮；未安装时显示「在线安装 / 从本地安装」与离线包按钮（`window.open`，与 TTS「获取密钥」一致）；**已安装时只保留卸载**，不重复展示本地安装与离线链接。
- 快捷键 `voiceInput`：提示需先安装模型。
- 麦克风/PTT 未装：toast + 打开设置 AI 并滚到该区块。
- 启动空闲预加载：仅当模型已可用时执行。
