...
- **微信过程消息 digest（progress buffer）**：`sendProcessMessages` 开启时，`IMService` 在 `beginOutboundSession` 传入 `bufferProgress`；实现 `IMProgressOutboundCapable` 的适配器（当前仅 `WeChatAdapter`）经 `wechat/outbound-progress.ts` 累积工具进度（🔧/❌）与中间 message 正文，**25s 定时**或 **满 12 行**或 **`flushProgress` 调用**（任务结束 / ask / confirm 前）合并为 digest。`IMService` 只调 `sendProgressText` / `sendProgressMarkdown` / `flushProgress` 可选方法，不感知微信平台细节。最终结果、任务错误、投递失败、ask/confirm 仍走 `sendText` / `sendMarkdown` 直发。
